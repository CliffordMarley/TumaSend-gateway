...
router.get("/", requireAuth, async (req, res) => {
	try {
		const { data: user, error } = await supabaseAdmin
			.from("users")
			.select(
				"id, email, full_name, phone, avatar_url, email_verified, phone_verified, status, created_at",
			)
			.eq("id", req.user.id)
			.single();
...
router.patch("/", requireAuth, async (req, res) => {
	...
	// Invalidate cached user record so new data is picked up on next request
	await invalidateUserCache(req.firebaseUser.uid);
